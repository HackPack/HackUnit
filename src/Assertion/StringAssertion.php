...
class StringAssertion implements \HackPack\HackUnit\Contract\Assertion\StringAssertion
{
    use FailureEmitter;
    use SuccessEmitter;
    ...
    public function is(string $expected) : this
    {
        if($this->context === $expected) {
            $this->negate ?
                $this->fail(Vector{
                    'Strings expected to be non-identical.'
                }) :
                $this->emitSuccess();
            return $this;
        }
    ...
    public function hasLength(int $length) : this
    {
        ...
        $this->negate ?
            $this->emitSuccess() :
            $this->fail(Vector{
                'Expected string to have length different than ' . $length,
            });
        return $this;
    }
    ...
    public function matches(string $pattern) : this
    {
        ...
        if($result === 1) {
            $this->negate ?
                $this->fail(Vector{
                    'String expected to not match ' . $pattern,
                    $this->context,
                }) :
                $this->emitSuccess();
            return $this;
        ...
        $this->negate ?
            $this->emitSuccess() :
            $this->fail(Vector{
                'String expected to match ' . $pattern,
                $this->context,
            });
        return $this;
    }
